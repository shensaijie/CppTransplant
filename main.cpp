	...
	virtual void run(const MatchFinder::MatchResult& Result)
	{
			...
			Result.Nodes.getNodeAs<clang::CallExpr>("call"))
		{
			...
			if (FuncName.size())
			{
				...
				Rewrite.ReplaceText(Call->getCallee()->getExprLoc(), FuncName.size(),
					FuncName);
			}
		}
	}
...
public:
	MyASTConsumer(Rewriter& R) : HandlerForIf(R), HandlerForFor(R), Renamer(R)
	{
		...
		// 替换类调用
		// Matcher.addMatcher(cxxRecordDecl(hasName("CArray")).bind("classDecl"),
		// &Renamer);
		// Matcher.addMatcher(cxxNewExpr(has(declRefExpr(to(cxxRecordDecl(hasName("CArray")))))).bind("newExpr"),
		// &Renamer);
		// Matcher.addMatcher(varDecl(hasType(namedDecl(hasName("CArray")))).bind("arrayDecl"),
		// &Renamer);

		// Matcher.addMatcher(cxxMemberCallExpr(callee(cxxMethodDecl(ofClass(hasName("CStringArray"))))).bind("memberCallExpr"),
		// new ClassRenamer_CStringArray(R));
		// Matcher.addMatcher(varDecl(hasType(namedDecl(hasName("CStringArray")))).bind("varDecl"),
		// new ClassRenamer_CStringArray(R));
...
int main(int argc, const char** argv)
{
	...
	AdjustedArgs.push_back(
		"-IE:\\AWorkSpace\\Eps2020\\include\\InterfaceInclude");
	...
	Tool.appendArgumentsAdjuster(
		getInsertArgumentAdjuster(AdjustedArgs, ArgumentInsertPosition::END));
